...
- **Intento de verificar arranque limpio bloqueado por política de Windows, no por el código.** Al clonar el repositorio en una carpeta nueva y seguir el README paso a paso, `scikit-learn` falló al cargar por un bloqueo de "Control Inteligente de Aplicaciones" de Windows sobre un DLL de `scipy` — una política de seguridad del sistema operativo local, no relacionada con el código ni las dependencias declaradas. Se descartó perseguir esta vía (la función no admite desactivarse sin reinstalar Windows) y se confirmó el correcto funcionamiento del servicio por otra vía: los 11 tests de `tests/test_contrato.py` pasan, y el servicio corre sin problemas en el entorno de desarrollo principal.